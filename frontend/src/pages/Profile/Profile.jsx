...
export default function Profile() {
  ...
  const [targets, setTargets] = useState({
    main: { stat_id: "", target_value: "", is_up: true, type_target: "main" },
    important: Array(4).fill(null).map(() => ({ stat_id: "", target_value: "", is_up: true, type_target: "important" }))
  });
  ...
  useEffect(() => {
    if (data?.user) {
      ...
      if (u.targets) {
        ...
        const important = u.targets.filter(t => t.type_target === "important");
        
        setTargets({
          main: main ? {
            stat_id: main.id_stat_name || "",
            target_value: main.target_value || "",
            is_up: Boolean(main.is_up),
            type_target: "main"
          } : { stat_id: "", target_value: "", is_up: true, type_target: "main" },
          important: Array(4).fill(null).map((_, i) => {
            const imp = important[i];
            return imp ? {
              stat_id: imp.id_stat_name || "",
              target_value: imp.target_value || "",
              is_up: Boolean(imp.is_up),
              type_target: "important"
            } : { stat_id: "", target_value: "", is_up: true, type_target: "important" };
          })
        });
      }
    }
  ...
  const handleSaveTargets = async () => {
    try {
      ...
      queryClient.invalidateQueries(["profileData"]);
    } catch (e) { alert("Ошибка при сохранении целей"); }
  };
  ...
  if (isError) return <div className="app-error">Ошибка: {error.message}</div>;

  const selectedIds = [targets.main.stat_id, ...targets.important.map(s => s.stat_id)].filter(Boolean);
  const availableStats = data?.available_stats || [];

  return (
    <div className={styles.layout}>
      {!isMobile && <div className={styles.sidebarDesktopWrapper}><Sidebar user={data.user} /></div>}
          ...
          <div className={styles.leftCol}>
            <section className={`${styles.card} ${styles.infoCard}`}>
              ...
              <div className={styles.scrollArea}>
                <div className={styles.field}><label>ФИО</label><input type="text" className={styles.input} value={info.name} onChange={e => setInfo({...info, name: e.target.value})} /></div>
                <div className={styles.formRow}>
                  <div className={styles.field}><label>Дата рождения</label><input type="date" className={styles.input} value={info.birth_day} onChange={e => setInfo({...info, birth_day: e.target.value})} /></div>
                  <div className={styles.field}><label>Рост (см)</label><input type="number" className={styles.input} value={info.height} onChange={e => setInfo({...info, height: e.target.value})} /></div>
                </div>
                ...
                <div className={styles.field}><label>Ограничения</label><textarea className={styles.textarea} value={info.diseases} onChange={e => setInfo({...info, diseases: e.target.value})} /></div>
              </div>
          ...
          <div className={styles.rightCol}>
            <section className={`${styles.card} ${styles.targetCard}`}>
              ...
              <div className={styles.targetList}>
                <div className={styles.targetGroup}>
                  ...
                  <TargetRow item={targets.main} stats={availableStats} selectedIds={selectedIds} onUpdate={(val) => setTargets({...targets, main: val})} />
                </div>
                <div className={styles.targetGroup}>
                  <span className={styles.labelSmall}>Важные показатели</span>
                  {targets.important.map((s, i) => (
                    <TargetRow key={i} item={s} stats={availableStats} selectedIds={selectedIds} onUpdate={(val) => {
            ...
            <section className={`${styles.card} ${styles.securityCard}`}>
              ...
              <div className={styles.actionButtons}>
                <button className={styles.btnOutline} onClick={() => alert('Функционал смены пароля')}>Сменить пароль</button>
    ...
    <div className={styles.targetRow}>
      <div className={styles.targetColSelect}>
        <select 
          ...
          onChange={e => onUpdate({ ...item, stat_id: e.target.value })}
        >
          <option value="">Выберите цель</option>
          {stats.map(s => {
            const isTaken = selectedIds.includes(s.id) && s.id !== item.stat_id;
            return !isTaken ? <option key={s.id} value={s.id}>{s.name}</option> : null;
          })}
        </select>
      </div>
