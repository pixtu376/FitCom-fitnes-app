...
export default function NutritionMemo({ data, isMobile = false }) {
  const pieData = [
  ...
  const stats = [
    { label: "Калории", val: `${data?.calories || 0} Ккал`, c: "#4ade80", p: 75 },
    { label: "Белки", val: `${data?.protein || 0} Г`, c: "#5383b1", p: 60 },
    { label: "Жиры", val: `${data?.fats || 0} Г`, c: "#e5a55d", p: 40 },
    { label: "Углеводы", val: `${data?.carbohydrates || 0} Г`, c: "#71c391", p: 85 },
  ];
            ...
            <div key={s.label} className={styles.row}>
              ...
              <div className={styles.barBg}>
                <div 
                  ...
                  style={{ width: `${s.p}%`, backgroundColor: s.c }} 
                />
              </div>
            </div>
          ))}
